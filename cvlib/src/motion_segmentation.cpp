...
void motion_segmentation::apply(cv::InputArray _image, cv::OutputArray _fgmask, double)
{
	...
	else
	{
		...
		cv::absdiff(curr_frame, mean, diff);
		cv::divide(diff,dev,div);
		
		cv::compare(div, cv::Scalar(k), F, cv::CMP_GT);
		...
		mean = p * curr_frame + (1 - p) * mean;
		cv::absdiff(curr_frame, mean, dist);
		dev = (p * dist.mul(dist) + (1 - p) * dev.mul(dev))^(1.0/2.0);
	}
